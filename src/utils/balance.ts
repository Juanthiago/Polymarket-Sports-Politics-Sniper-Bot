...
export async function validateBuyOrderBalance(
  ...
  try {
    ...
    if (!valid) {
      console.log(`Balance: need $${requiredAmount.toFixed(2)}, have $${availableUsd.toFixed(2)} (bal $${balanceUsd.toFixed(2)}, allow $${allowanceUsd.toFixed(2)})`);
    }
    return { valid, available: availableUsd, required: requiredAmount };
  } catch (error) {
    const msg = error instanceof Error ? error.message : String(error);
    console.log(`Balance check: ${msg}`);
